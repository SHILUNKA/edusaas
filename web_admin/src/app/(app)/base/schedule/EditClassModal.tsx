...
export default function EditClassModal({ token, classData, onClose, onSuccess }: EditClassModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-md">
            <div className="bg-gradient-to-br from-white to-slate-50/30 rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] relative border border-slate-100">

                {/* Header - Soft UI */}
                <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-purple-50">
                    <div>
                        <h3 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent flex items-center gap-2">
                            ...
                            {isExpired && <span className="bg-slate-200 text-slate-600 text-xs px-3 py-1 rounded-2xl flex items-center gap-1 shadow-sm"><Lock size={12} /> 已结束</span>}
                        </h3>
                                        ...
                                        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
                                            ...
                                            {loadingCards ? (
                                                ...
                                                <>
                                                    ...
                                                    <button onClick={handleEnroll} disabled={loading} className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2.5 rounded-2xl text-sm font-bold hover:shadow-lg hover:shadow-indigo-300/50 shadow-md disabled:opacity-50 transition-all hover:scale-105">{loading ? '提交中...' : '确认添加'}</button>
                                                </>
                                            ) : (
