...
// 引入子组件
import BossDashboard from './tenant-roles/BossDashboard';
import FinanceDashboard from './tenant-roles/FinanceDashboard';
import OpsDashboard from './tenant-roles/OpsDashboard';
import HRDashboard from './tenant-roles/HRDashboard';
...
export default function TenantDashboardView({ roles = [] }: { roles: string[] }) {
    ...
    // 权限判断
    const isBoss = roles.includes('role.tenant.admin');
    const isFinance = roles.includes('role.tenant.finance');
    const isOps = roles.includes('role.tenant.operation');
    const isHR = roles.includes('role.tenant.hr');
        ...
        async function fetchData() {
            ...
            try {
                ...
                const [resBasic, resAdv, resStaff] = await Promise.all([
                    fetch(`${API_BASE_URL}/tenant/dashboard/stats`, { headers }),
                    fetch(`${API_BASE_URL}/tenant/dashboard/analytics`, { headers }),
                    fetch(`${API_BASE_URL}/tenant/dashboard/pending-staff`, { headers })
                ]);
